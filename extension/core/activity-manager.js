...
const SECRET_FIELD = /token|secret|cookie|authorization|oauth|credential|password|capability|nonce/i;
const ACTIVITY_CAPABILITY_PATTERN = /^[a-f0-9]{64}$/;
...
function createActivityCapability() {
  if (typeof globalThis.crypto?.getRandomValues !== 'function') {
    throw new Error('Secure randomness is unavailable for Activity ownership.');
  }
...
function redactActivityCapability(value, capability) {
  if (typeof value === 'string') {
    return ACTIVITY_CAPABILITY_PATTERN.test(capability || '')
      ? value.split(capability).join('[redacted capability]')
      : value;
  }
  if (Array.isArray(value)) return value.map((entry) => redactActivityCapability(entry, capability));
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, entry]) =>
      [key, redactActivityCapability(entry, capability)]));
  }
  return value;
}
...
function scriptDefinition(record, extensionVersion = '0.0.0') {
  ...
  const code = [
    ...
    '  let __runtimeReconnectAttempts = 0;',
    ...
    '    report(report) { __lastReport = report; return __request("presence.report", { report }); },',
    '    clear() { __lastReport = null; return __request("presence.clear"); },',
    ...
    '  const __connectRuntime = () => {',
    '    if (__lifecycleEnded) return;',
    '    try {',
    '      const port = chrome.runtime.connect({ name: `chudpresence-activity-v1:${__activityIdentity.activityId}:${__activityIdentity.activityVersion}:${__activityIdentity.capability}` });',
    '      __runtimePort = port;',
    ...
    '        if (__lifecycleEnded || __runtimePort !== port) return;',
    '        __runtimePort = null;',
    '        if (++__runtimeReconnectAttempts >= 10) __endLifecycle("runtime-disconnected");',
    '        else __lifecycle.timeout(__connectRuntime, 500);',
    '      });',
    '      __lifecycle.timeout(() => {',
    '        if (__runtimePort !== port || __lifecycleEnded) return;',
    '        __runtimeReconnectAttempts = 0;',
    '        if (__lastReport) void __request("presence.report", { report: __lastReport }).catch(() => {});',
    '      }, 1000);',
    '    } catch {',
    '      if (++__runtimeReconnectAttempts >= 10) __endLifecycle("runtime-disconnected");',
    '      else __lifecycle.timeout(__connectRuntime, 500);',
...
export class ActivityManager {
    ...
    return Object.values(records).filter((record) =>
      ...
      const compatibilityStatus = !isSupportedActivityApiVersion(record.metadata.apiVersion)
        ...
        : record.metadata.minExtensionVersion && compareActivityVersions(runtimeVersion, record.metadata.minExtensionVersion) < 0
          ...
          : '';
      return redactActivityCapability({
  ...
  async handleUserScriptConnect(port) {
    ...
    const portMatch = /^chudpresence-activity-v1:([a-z0-9][a-z0-9-]{1,63}):([^:]+):([a-f0-9]{64})$/.exec(port?.name || '');
    if (!portMatch) return false;
    const [, portActivityId, activityVersion, capability] = portMatch;
    let id = portActivityId;
    if (typeof worldId === 'string') {
      if (!worldId.startsWith(WORLD_ID_PREFIX)) return false;
      id = worldId.slice(WORLD_ID_PREFIX.length);
      if (id !== portActivityId) return false;
    }
    const record = (await this.load())[id];
    if (!record || record.metadata.id !== id || !record.enabled ||
        activityVersion !== record.metadata.version || capability !== record.capability ||
  ...
  async register(record) {
    ...
    if (typeof this.api.userScripts?.execute !== 'function') {
      throw new Error('Chrome does not expose the required userScripts.execute API. Use Chrome 135 or newer; on Chrome 138 or newer, enable Allow User Scripts on the extension details page.');
    }
    if (!await hasUserScriptsPermission(this.api)) {
      throw new Error('The userScripts permission is unavailable. Reload the extension and enable Allow User Scripts in Chrome extension details.');
    }
  ...
  async restoreAllInternal() {
    ...
    for (const [key, record] of Object.entries(records)) {
      ...
      if (!ACTIVITY_CAPABILITY_PATTERN.test(record.capability || '')) {
        record.capability = createActivityCapability();
      }
      if (record.metadata.minExtensionVersion &&
          compareActivityVersions(extensionVersion(this.api), record.metadata.minExtensionVersion) < 0) {
  ...
  async handleUserScriptMessage(message, sender) {
    ...
    const records = await this.load();
    const worldId = sender?.userScriptWorldId;
    let id = message?.activityId;
    if (typeof worldId === 'string') {
      if (!worldId.startsWith(WORLD_ID_PREFIX)) return false;
      const worldActivityId = worldId.slice(WORLD_ID_PREFIX.length);
      if (worldActivityId !== id) return false;
      id = worldActivityId;
    }
    if (!isActivityId(id) || !ACTIVITY_CAPABILITY_PATTERN.test(message?.capability || '')) return false;
    const record = records[id];
